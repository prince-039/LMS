...
const register = async (req, res, next) => {
  ...
  const user = await User.create({
    ...
    avatar: {
        public_id: email,
       secure_url: 'https://res.cloudinary.com/du9jzqpt/image/upload/v1712341234/user_avatar_123.jpg'
     ...
     if(req.file) {
      ...
      try {
           ...
           if(result) {
            ...
            // Remove the file from local storage(server)
...
const getProfile = async (req, res, next) => {
  ...
  const message = `You can reset your password by clicking <a href="${resetPasswordUrl}" target="_blank">Reset your password</a>.\nIf the above link does not work, then copy and paste this link in a new tab: ${resetPasswordUrl}.\nIf you did not request this, kindly ignore this email.`;
    ...
    if(req.file) {
      await cloudinary.v2.uploader.destroy(user.avatar.public_id);
       try {
        ...
        return next(new AppError(error ||'Failed to upload avatar, please try again', 500));
      }
    }
    await user.save();
